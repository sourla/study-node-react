...
- 테스트 파일은 대상 파일 옆에 같은 이름으로 둔다. loader 단위 테스트는 `ssrLoader.test.ts`처럼 `.test.ts`, 컴포넌트 렌더링 테스트는 `SsrPage.test.tsx`처럼 `.test.tsx`.
- Zod 스키마의 `createdAt`은 `z.iso.datetime()`이라 fixture는 `'2026-09-03T00:00:00.000Z'` 형식이어야 통과한다.
